...
public class MainView extends VerticalLayout {
    ...
    public void start() {
        ...
        var start = new Button("Посетить библиотеку", e -> {
            removeAll();
            var library = new Label("Домашняя библиотека");
            var buttonsLayout = new HorizontalLayout();

            var take = new Button("Взять книгу", event -> {
                var takingLabel = visitorService.takeBook();
                add(takingLabel);
            });

            var add = new Button("Добавить книгу", event -> {
                ...
                var addBook = new Button("Добавить", clickEvent -> {
            ...
            var rate = new Button("Оценить книгу", event -> {
                var ratingLabel = visitorService.rateBook();
                add(ratingLabel);
            });

            var takeBack = new Button("Вернуть книгу", event -> {
                var returningLabel = visitorService.returnBook();
                add(returningLabel);
            });

            var delete = new Button("Удалить книгу", event -> {
                var bookName = new TextField("Название");
                var ownerLastName = new TextField("Фамилия владельца");

                var deleteBook = new Button("Удалить", clickEvent -> {
                    var owner = profilesService.find(ownerLastName.getValue());
